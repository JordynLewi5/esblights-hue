...
class Light {
  ...
  async scheduleLight() {
    ...
    if (scheduleEvaluated[0] < scheduleEvaluated[1]) {
      if (
        Util.getCurrentTimeInDecimal() > scheduleEvaluated[0] &&
        Util.getCurrentTimeInDecimal() < scheduleEvaluated[1]
      ) {
        this.setPower(true);
    ...
    const timeUntilOn =
      scheduleEvaluated[0] > Util.getCurrentTimeInDecimal()
        ? scheduleEvaluated[0] - Util.getCurrentTimeInDecimal()
        : scheduleEvaluated[0] - Util.getCurrentTimeInDecimal() + 24;
    const timeUntilOff =
      scheduleEvaluated[1] > Util.getCurrentTimeInDecimal()
        ? scheduleEvaluated[1] - Util.getCurrentTimeInDecimal()
        : scheduleEvaluated[1] - Util.getCurrentTimeInDecimal() + 24;
  ...
  async updateLights(bodyIn) {
    try {
      return await axios
        .put(
          `http://${this.lightInfo.bridgeIP}/api/${this.lightInfo.username}/lights/${this.lightInfo.id}/state`,
