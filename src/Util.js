...
class Util {
  ...
  static async getColorData() {
    return (
      await axios.get(
        `https://esblight-api.kinetic.com/api/esb-light-data?apikey=${process.env.ESBAPIKEY}`
      )
    ).data.content;
  }
  ...
  static async getSunset() {
    ...
    return await axios
      .get(
        `https://api.sunrisesunset.io/json?lat=33.500280&lng=-86.792912&&date=${year}-${month}-${day}`
      )
      .then((response) => {
        return (
          parseFloat(response.data.results.sunset.split(":")[0]) +
          parseFloat(response.data.results.sunset.split(":")[1]) / 60 +
          12
        );
      });
  }
